...
// MCPO 服務的端口與基礎 URL
let MCPO_PORT = 7861;
let MCPO_BASE = `http://localhost:7861`;

function setMcpoBase(url) {
    if (url) {
        // 如果已經包含 http:// 或 https://，則直接使用；否則預設使用 http://
        MCPO_BASE = url.includes('://') ? url : `http://${url}`;
        if (MCPO_BASE.endsWith('/')) MCPO_BASE = MCPO_BASE.slice(0, -1);
    }
}
...
async function buildMcpoToolMap() {
    if (mcpoToolMap !== null && !buildPromise) return mcpoToolMap;
    ...
    buildPromise = (async () => {
        ...
        try {
            ...
            let servers = Object.keys(rootResp.data.paths || {})
                .map(p => p.split('/')[1])
                .filter((v, i, a) => v && a.indexOf(v) === i);

            if (servers.length === 0 && rootResp.data.info && rootResp.data.info.description) {
...
module.exports = {
    get MCPO_BASE() { return MCPO_BASE; },
    setMcpoBase,
    buildMcpoToolMap,
    indexMcpoServer,
    getOpenAiTools
