...
const { clearHistory, loadHistory, saveHistory, getHistoryFiles, HISTORY_DIR } = require('./chat.js');
const mcpo = require('./chat/mcpo');
const fs_tool = require('../tool/fs');
...
async function memory(message, args) {
    ...
    const config = await fs_tool.read(configPath) || {};
    let currentConfig = { mcpo_ip: mcpo.MCPO_BASE, apikey: '' };

    if (message.guild && config[message.guild.name]) {
        currentConfig = config[message.guild.name];
        if (currentConfig.mcpo_ip) {
            mcpo.setMcpoBase(currentConfig.mcpo_ip);
        }
    }
    ...
    const authHeaders = currentConfig.apikey ? { 'Authorization': currentConfig.apikey } : {};
...
\`//memory mcp list\` - 顯示保存的 MCP 記憶 (不完整顯示)`;
    ...
    try {
        if (!isNaN(parseInt(subCmd)) && args[0].match(/^\d+$/)) {
            ...
            let relations = [];
            try {
                const graphResp = await axios.post(`${mcpo.MCPO_BASE}/memory/read_graph`, {}, {
                ...
                consol.warn(`[memory mcp] 無法讀取: ${err.message} (URL: ${mcpo.MCPO_BASE}/memory/read_graph)`);
                return await message.channel.send(`⚠️ 無法連線至 MCP memory server（或是 API 金鑰無效）\nURL: \`${mcpo.MCPO_BASE}\``);
            }
            ...
            if (mcpAction === 'list_full' || mcpAction === 'list') {
                ...
                try {
                    await axios.post(`${mcpo.MCPO_BASE}/memory/delete_entities`, { entityNames }, {
                        headers: { ...authHeaders, 'Content-Type': 'application/json' },
                        timeout: 15000
                    });
