      ...
      if (cmd[command]) {
        ...
        try {
          if (command == "redocmd") {
            ...
            // 自動記錄最近一次互動的頻道 ID 作為 IoT 警報頻道
            try {
              const discordDataPath = path.join(__dirname, 'data', 'discorddata.json');
              const discordData = await fs.read(discordDataPath) || {};
              if (discordData.iotAlertChannelId !== message.channel.id) {
                discordData.iotAlertChannelId = message.channel.id;
                await fs.write(discordDataPath, discordData);
                consol.info(`[IoT 設定] 已自動將最近使用的頻道 (${message.channel.id}) 設為警報通知頻道`);
              }
            } catch (err) {
              consol.warn(`[IoT 設定] 無法更新頻道 ID: ${err.message}`);
            }
