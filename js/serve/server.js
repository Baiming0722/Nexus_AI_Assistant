...
const { chat, getHistoryFiles, clearHistory, resetHistoryIndex, setHistoryIndex, loadHistory, getModelsList, getCurrentModel, setModel } = require('../cmd/chat.js');
...
var io = new Server(SerVer, {
    cors: {
        origin: function (origin, callback) {
            // 動態允許來自 IPFS、本機開發端點 (localhost/127.0.0.1)、Tauri 的跨域連線
            if (!origin || /localhost|127\.0\.0\.1/.test(origin) || /tauri:\/\//.test(origin) || /ipfs/.test(origin) || /filegear-sg\.me/.test(origin)) {
...
const sessionMiddleware = session({
    ...
    cookie: {
        // sameSite: 'none',   // 允許跨站傳遞 Cookie (例如 Tauri <-> filegear-sg)
        // secure: true        // 搭配 sameSite: 'none' 必須開啟
        // 本地開發時，請將 secure 關閉，否則 http:// 連線下瀏覽器不會儲存 Cookie
        secure: false
    }
});
async function server() {
    ...
    // 啟動智能車 MQTT 監控服務（注入 Socket.IO 實例以廣播資料）
    startCarService(io).catch(err => consol.error('[Server] 啟動車輛服務失敗:', err.message));
    ...
    app.get('/nkust', (req, res) => {
        res.redirect("https://nkust.dctggest.filegear-sg.me/index.html");
    });
    app.get('/chat', (req, res) => {
        res.sendFile(path.join(__dirname, '../web/chat.html'));
    });
    app.get('/login', (req, res) => {
        const { clientId, REDIRECT_URI } = require('../data/discorddata.json');
        console.log("登入重定向 URI:", REDIRECT_URI);
        ...
        const authorizationUrl = `https://discord.com/oauth2/authorize?client_id=${clientId}&response_type=code&redirect_uri=${encodeURIComponent(REDIRECT_URI)}&scope=identify+guilds+email+guilds.members.read+guilds.join&state=${state}`;
    ...
    app.get("/callback", async (req, res) => {
        ...
        console.log("Server.js 設定的重定向 URI:", REDIRECT_URI);
        const userData = await user.login(code, res);
        
        if (userData && userData.userinfo) {
            req.session.userId = userData.userinfo.id;
            req.session.save(() => {
            ...
            try {
                userid[userData.userinfo.id] = userData;
                fs.write(path.join(__dirname, '../data/user.json'), userid);
            } catch (error) {
...
io.on("connect", async (socket) => {
    ...
    socket.on('chat', async (msg, name) => {
        // 將當前使用者的完整 userinfo 傳入 chat()，供個人化記憶使用
        const currentUserData = userid[session.userId] || null;
        var result = await chat(null, msg, name, null, currentUserData);
    ...
    socket.on('new_chat', async () => {
        ...
        socket.emit('history_list', files);
    });
