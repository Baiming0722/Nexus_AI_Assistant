...
(function () {
    ...
    function initTrashbinChart() {
        ...
        trashbinChart = new Chart(ctx, {
            ...
            options: {
                ...
                scales: {
                    x: {
                        ticks: { maxTicksLimit: 5, color: '#9ca3af', font: { size: 5 } },
    ...
    function updateTrashbinDashboard(data) {
        ...
        if (statusEl) {
            statusEl.textContent = alertActive ? `⚠️ 滿桶請清潔 (${full_bucket || '未知'})` : '✅ 狀態正常';
            statusEl.style.color = getStatusColor(alertActive);
        }

        // — 新感測器狀態 —
        const lastCatEl = document.getElementById('trashbin-last-category');
        if (lastCatEl) lastCatEl.textContent = category_zh || '--';

        const fillEl = document.getElementById('trashbin-fill-percent');
        if (fillEl) fillEl.textContent = fill_percent !== undefined ? `${fill_percent}%` : '--%';

        const distEl = document.getElementById('trashbin-distance');
        if (distEl) distEl.textContent = distance_cm !== undefined ? `${distance_cm.toFixed(2)} cm` : '-- cm';

        const objEl = document.getElementById('trashbin-object-present');
        if (objEl) {
            objEl.textContent = object_present ? '有物體' : '無物體';
            objEl.style.color = object_present ? '#fb923c' : '#9ca3af';
        }
