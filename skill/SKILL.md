...
**描述**: 讀取本地 IoT 智慧垃圾桶資料（`car_status.json` / `car_log.json`），產生即時狀態摘要（包含最後辨識、容量佔比、超音波距離、物體偵測）、歷史趨勢分析與異常提醒。（本功能為 Script Skill，由 Python 腳本執行）。

**參數**:

- `data_dir` (str, 選填, 預設=`js/data/car/`): JSON 資料目錄路徑
- `log_limit` (int, 選填, 預設=20): 歷史趨勢分析的最大筆數

**執行邏輯**:

1. 讀取 `skills/iot_car/iot_car.md` 了解 Skill 的功能規格
2. 執行 `skills/iot_car/Script/iot_car.py` 腳本
3. 腳本讀取 `car_status.json`（最新狀態）與 `car_log.json`（歷史紀錄）
4. 輸出格式化的狀態摘要、趨勢分析，並依規則觸發提醒（滿桶 / 離線 / 初始化）
